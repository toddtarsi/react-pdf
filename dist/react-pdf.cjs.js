...
const inheritedProperties = ['color', 'fontFamily', 'fontSize', 'fontStyle', 'fontWeight', 'letterSpacing', 'opacity', 'textDecoration', 'lineHeight', 'textAlign', 'textTransform', 'visibility', 'wordSpacing'];
...
class Base extends Node {
  ...
  applyProps() {
    ...
    ramda.toPairsIn(this.style).map(([attribute, value]) => {
  ...
  resolveStyles() {
    ...
    const inheritedStyles = this.parent ? ramda.pick(inheritedProperties, this.parent.style) : {};
...
const getFontSize = ramda.pathOr(12, ['attributes', 'fontSize']);
...
const fontSubstitution = () => ({
  ...
  if (lastIndex < string.length) {
    const fontSize = getFontSize(ramda.last(runs));
    res.push({
      start: lastIndex,
      end: string.length,
      attributes: {
...
const isImage = ramda.propEq('name', 'Image');
const isTextInstance = ramda.compose(ramda.complement(ramda.isNil), ramda.prop('value'));
...
const emptyObject = {}; // If the Link has a string child or render prop, substitute the instance by a Text,
